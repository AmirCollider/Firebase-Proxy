...
export const CONFIG = deepFreeze({
  ...
  DOCSNAP: {
    ...
    //
    // Everything else on the product page is a CLAIM about what the
    // output looks like; the videos are a claim that moves. A link
    // to the output itself is the claim and the evidence in one
    // click, and it is the only thing on that page that costs a
    // stranger nothing at all to check - no install, no account,
    // no download. It is also the one asset that can be pasted
    // into an email or a forum reply without asking the reader to
    // install a package first.
    //
    // Null until an export is actually uploaded, because a demo
    // button that 404s costs more trust than no demo button loses.
    // While this is null the page renders nothing for it.
    //
    // To fill it in: run Export Full Project on a small, tidy demo
    // project, upload the whole output folder to the R2 bucket
    // under `demo/docsnap/`, open it once to confirm, then set
