...
export const CONFIG = deepFreeze({
  ...
  DOCSNAP: {
    ...
    // A real export - generated by the tool, hosted here - that a
    // visitor can click around before installing anything.
    //
    // This is the one thing the product page cannot argue its way
    // to. Everything else on that page is a claim about what the
    // output looks like; a link to the output itself is the claim
    // and the evidence in the same click, and it is the asset a
    // reviewer, a newsletter or a forum reply can be handed
    // without asking anybody to install a package first.
    //
    // Null until one is actually uploaded, because a demo button
    // that 404s costs more trust than no demo button buys. The
    // page renders nothing at all while this is null.
    //
    // To fill it in: run Export Full Project on a small, tidy
    // demo project, upload the whole output folder to the R2
    // bucket under `demo/docsnap/`, confirm the index opens, then
    // set this to '/assets/demo/docsnap/index.html'. Nested keys
    // under /assets/ are served (see Api/AssetApi.js), so the
  ...
  COMMERCE: {
    ...
    // ==========================================
    // Whether the provider's fiat on-ramp is switched on for this
    // account.
    //
    // NOWPayments can put a card step in front of its own invoice
    // - Visa, Mastercard, Apple Pay, Google Pay - with a partner
    // doing the conversion. The buyer pays in their own currency
    // and never touches an exchange; the money still arrives here
    // as crypto. For a thirty-dollar editor extension sold to
    // developers who mostly hold no crypto at all, that is the
    // difference between a checkout and a wall.
    //
    // It is a flag rather than something the page works out for
    // itself, because there is nothing to work it out FROM: the
    // on-ramp is an account setting in the provider's dashboard
    // and the invoice this Worker creates is byte-identical
    // either way. So the page has to be told.
    //
    // The cost of getting it wrong is asymmetric, which is why
    // the default is off. A page that undersells a card field the
    // invoice does have loses nothing but a little conversion. A
    // page that PROMISES a card field the invoice does not have
    // sends somebody who cannot pay in crypto all the way to a
    // payment screen to find that out - and that person does not
    // come back, and does not write in to say why.
    //
    // Turn it on only after opening a real invoice and seeing the
    // card option with your own eyes:
    //   NOWPayments dashboard - Settings - Payments - Fiat on-ramp
    FIAT_ONRAMP: false,

    // Offer a way to reach a human when the payment page is not an
    // option at all: no card the on-ramp will take, no crypto, a
    // country the provider does not reach, or a buyer who would
    // simply rather pay some other way. It links to the contact
    // form with the order topic already chosen, and the key is
    // then minted by hand (Docs/Licensing.md - Generate a batch).
    //
    // On by default, because the alternative is that all of those
    // people just leave. A checkout cannot tell the difference
    // between somebody who decided not to buy and somebody who
    // could not find a way to - and it is the second group that
    // is worth a link.
    ALT_PAY: true
  },
