...
function renderGameFacts(lang) {
  const facts = gameFacts(CONTEXT.siteLevel ? null : CONTEXT.game, lang, 'privacy')
  if (!facts) return ''

  return `
      <section class="panel">
        <h2><span class="sec-ic">${icon('clipboard', 'p-ic')}</span><span>${escapeHtml(facts.title)}</span></h2>
